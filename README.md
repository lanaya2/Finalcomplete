...
11/11/2025 ADDED:
Login feature using Firebase, (use "devuser@twu.edu  &  dev0nly" for a temp student login ||  "dransom2@twu.edu  &  123456" for admin login), X
extracted login functions from test.html and injected into login.html X
redirects from login to temporary admin or student pages respectively X
...
TODO:
    Ensure users cant overwrite already created test, by either providing a uniqueID or unique test names only.
    ...
    Ask if teacher/student on signup X
    Scrub older files to assure they match the changed logic from todays code X
...
(LAST DAY)
TODO:
    Users only use twu email (maybe) X
    ((await verification to login)) (unlikely) [yeah not today]
    User lookup (inbox) (maybe) X
    Quiz lookup (quiz_take) X 
    UI Improvements (maybe) X
    Test cant be overwritten (NEEDED) X
    Student inbox X
    Add option to delete recieved messages (everyone) X
    Viewing Quizes(NEEDED) X
    Overwritten data preventitave measures overall X
    Bug testing 

    Teachers view/delete quizzes X
    Teachers view student scores X
    Search bar X